...
let fullName = [];
 runners.forEach((name)=>{
     let names =` FirstName:${name.first_name} LastName:${name.last_name}`
    return fullName.push(names)} )
...
allCaps = runners.map((caps)=>{
  ...
  let CapfirstName = ` FirstName:${caps.first_name.toUpperCase()}  LastName:${caps.last_name}`
   return CapfirstName
})
console.log(allCaps); 
...
largeShirts = runners.filter((size)=>{ return size.shirt_size === 'L'
...
// Problem 1
// Return an array named TwoXL that contains information about the runners that have a shirt size of 2XL and donation is less than 80 and log the result
let TwoXL =[]
 TwoXL= runners.filter( (shirt) => { return shirt.shirt_size = '2XL' && shirt.donation < 80 })

console.log(TwoXL)

// Problem 2 show email and first name
let Capital = []

Capital = runners.map( (cap)=>{
    let first = cap.first_name;
    let email = cap.email;
    return ` FirstName:${first} Email:${email}`
})
console.log(Capital)
...
runners.forEach( (company)=>{
  ...
  let lastName=  company.last_name;
  
  let comapanyInfo= ` Company:${companyName}  LastName:${lastName}`;

  return companyLastname.push(comapanyInfo);
})

console.log(companyLastname)